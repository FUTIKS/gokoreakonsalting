...
const Index = () => {
  ...
  const { ref: videosInViewRef, inView: isVideosInView } = useInView({ threshold: 0.3, triggerOnce: false });
  const { ref: faqInViewRef, inView: isFaqInView } = useInView({ threshold: 0.3, triggerOnce: false });
  ...
  const setVideosRefs = useCallback((node: HTMLDivElement | null) => {
    videosScrollRef.current = node;
    videosInViewRef(node);
  }, [videosInViewRef]);

  const setFaqRefs = useCallback((node: HTMLDivElement | null) => {
    faqScrollRef.current = node;
    faqInViewRef(node);
  }, [faqInViewRef]);
      ...
      <div className="min-h-screen bg-black flex flex-col">
        ...
        <div className="relative z-10 flex-grow">
          ...
          <main>
            ...
            <div ref={setFaqRefs}>
              <SectionWrapper>
                <FAQSection onContactClick={() => openConsultationModal()} />
              </SectionWrapper>
            </div>
